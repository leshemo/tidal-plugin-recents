...
const persistentStore = (await ReactiveStore.getPluginStorage(
  "SortByRecentlyPlayed",
  { [RECENTLY_PLAYED_KEY]: [] },
)) as Record<string, any>;
let recentlyPlayedOrder: string[] =
  (persistentStore[RECENTLY_PLAYED_KEY] as string[]) || [];
...
MediaItem.onMediaTransition(unloads, async (mediaItem: any) => {
  ...
  if (consecutiveCount === trackThreshold) {
    ...
    recentlyPlayedOrder = [
      albumId,
      ...recentlyPlayedOrder.filter((id) => id !== albumId),
    ];
...
function sortAlbumsByRecentlyPlayed(albums: any[]): any[] {
  ...
  return [...albums].sort((a, b) => {
    const aIdx = orderMap.has(a.id.toString())
      ? orderMap.get(a.id.toString())!
      : Infinity;
    const bIdx = orderMap.has(b.id.toString())
      ? orderMap.get(b.id.toString())!
      : Infinity;
  ...
  (action: any) => {
    ...
    try {
      ...
      const recentlyPlayedInPage = sortedItems
        .filter((album: any) =>
          recentlyPlayedOrder.includes(album.id.toString()),
        )
        .slice(0, 3)
